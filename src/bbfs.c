...
static void bb_bkend_path(char fpath[PATH_MAX], const char *path)
{
    strcpy(fpath, BB_DATA->bkend_dir);
    strncat(fpath, path, PATH_MAX); // ridiculously long paths will
				    // break here

    log_msg("    bb_bkend_path:  bkend_dir = \"%s\", path = \"%s\", fpath = \"%s\"\n",
	    BB_DATA->bkend_dir, path, fpath);
}
...
int bb_rename(const char *path, const char *newpath)
{
    ...
    log_msg("\nbb_rename(fpath=\"%s\", newpath=\"%s\")\n",
	    path, newpath);
    bb_bkend_path(fpath, path);
    bb_bkend_path(fnewpath, newpath);
	       ...
	       struct fuse_file_info *fi)
{
    ...
    // once again, no need for bkend_path -- but note that I need to cast fi->fh
...
int main(int argc, char *argv[])
{
    ...
    bb_data->bkend_dir = malloc(sizeof(char) * PATH_MAX);
    strcpy(bb_data->bkend_dir, bb_data->rootdir);
    strncat(bb_data->bkend_dir, "/.p4_bckend/", PATH_MAX);
